...
const BACKEND_URL = `${process.env.LINKIFY_BACKEND_URL}`;
...
export const backendFetch = (endpoint, parameters,  onSuccessCallback, onErrorCallback) => {
    const response = fetch(`${BACKEND_URL}${endpoint}`, parameters);
    response.then( results => handleResponse(results, onSuccessCallback, onErrorCallback))
            .catch(this.onServiceErrorCallback)
}
 ...
 */
export const setOnServiceErrorCallback = (callback) => {onServiceErrorCallback = callback};
